...
import { apiGetOrder } from '@/api/index'
...
const {status, data: orderInfo} = await apiGetOrder(bookingId, token) 
// console.log("訂單Id=>",orderInfo);

const {countDay} = useDayNum()
const totalDays = ref(0)
if(status.value === "success"){
  const startDate = orderInfo.value.checkInDate.split('T')[0];
  const endDate = orderInfo.value.checkOutDate.split('T')[0];
  totalDays.value = countDay(startDate,endDate)
      ...
      <div class="row gap-15 gap-md-0">
        <div class="col-12 col-md-7">
          ...
          <div class="d-flex flex-column align-items-md-start">
            ...
            <NuxtLink :to="`/user/${userInfo._id}/order`"
          ...
          <div
            ...
            <section class="d-flex flex-column gap-6">
              <h3 class="d-flex align-items-center mb-6 text-neutral-80 fs-8 fs-md-6 fw-bold">
                <p class="mb-0">
                  {{orderInfo.roomId.name}}，{{totalDays}}晚
                </p>
                <span
              ...
              <div class="text-neutral-80 fs-8 fs-md-7 fw-bold">
                <p class="title-deco mb-2" v-dayFormat="orderInfo.checkInDate">
                  入住：[day]，15:00 可入住~
                </p>
                <p class="title-deco mb-0" v-dayFormat="orderInfo.checkOutDate">
                  退房：[day]，12:00 前退房~
                </p>
              </div>
 
              <p class="mb-0 text-neutral-80 fs-8 fs-md-7 fw-bold">
                NT$ {{ orderInfo.roomId.price * totalDays }}
              </p>
            </section>
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <template v-for="(facility,idx) in orderInfo.roomId.facilityInfo">
            ...
            <section>
              ...
              <ul class="d-flex flex-wrap row-gap-2 column-gap-10 p-6 mb-0 fs-8 fs-md-7 bg-neutral-0 border border-neutral-40 rounded-3 list-unstyled">
                <template v-for="(amenity,idx) in orderInfo.roomId.amenityInfo">
