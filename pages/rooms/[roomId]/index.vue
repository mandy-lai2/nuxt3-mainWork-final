...
const computedPrice = computed(() => {
  console.log(daysCount.value, roomInfo.value.price); 
...
const onSubmit = async ()=>{
 ...
 setBookingLatest({orderInfo, daysCount, roomInfo})
...
onMounted(() => {
  // console.log("onmounted->",bookingLatest.value);
  if(bookingLatest.value.orderInfo?.roomId === roomId){
    daysCount.value = bookingLatest.value.daysCount
    bookingDate.date.start = bookingLatest.value.orderInfo.checkInDate
    bookingDate.date.end = bookingLatest.value.orderInfo.checkOutDate
    ...
    <section class="py-10 py-md-30 bg-primary-10">
        ...
        <div class="row">
          ...
          <div class="d-none d-md-block col-md-5">
            ...
            >
              ...
              <VForm v-slot="{ errors, meta }" @submit="onSubmit">
              <div>
      ...
      <div class="d-flex d-md-none justify-content-between align-items-center position-fixed bottom-0 w-100 p-3 bg-neutral-0">
        ...
        <template v-else>
          ...
          <button type="button" @click="onSubmit" class="btn btn-primary-100 px-12 py-4 text-neutral-0 fw-bold rounded-3" >立即預訂</button>
        </template>
      </div>
    </section>
    <ClientOnly>
